...
class NotificationService:
    ...
    def _init_services(self):
        """Initialize and validate notification providers (Telegram only)."""
        # Safe default so downstream attribute access never raises.
        self.telegram_bot = None

        # Telegram initialization
        if token := _get_env("TELEGRAM_TOKEN"):
            try:
                self.telegram_bot = FlareGuardBot(
                    token, _get_env("TELEGRAM_CHAT_ID"))
    ...
    def send_alert(self, frame, caption: str) -> bool:
        """Non-blocking alert dispatch. `caption` is the fully-formatted message."""
        image_path = self.save_frame(frame)

        future = self.executor.submit(
            self._send_alerts_async, image_path, caption
        )
        future.add_done_callback(
            lambda f: f.exception() and logger.error(f"Alert error: {f.exception()}")
        )
        return True  # dispatched (delivery is logged asynchronously)

    def _send_alerts_async(self, image_path, caption):
        """Background alert processing (Telegram)."""
        if self.telegram_bot:
            self._send_telegram_alert(image_path, caption)

    def _send_telegram_alert(self, image_path, caption):
    ...
    def send_test_message(self):
        """Verify system connectivity (Telegram)."""
        success = False
        if self.telegram_bot:
            try:
